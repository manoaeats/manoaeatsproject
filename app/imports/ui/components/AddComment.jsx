...
class AddComment extends React.Component {
  ...
  submit(data, formRef) {
    const { comment, rating, vendorId, createdAt } = data;
    const owner = Meteor.user().username;
    Comments.collection.insert({ comment, rating, vendorId, createdAt, owner },
        (error) => {
          if (error) {
  ...
  render() {
    ...
    return (
        <AutoForm ref={ref => { fRef = ref; }} schema={bridge} onSubmit={data => this.submit(data, fRef)} >
          <Segment>
            <TextField label="Add a review" name='comment'/>
            <div>
